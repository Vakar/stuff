package space.vakar.stuff.persistence.impl.hql;

import space.vakar.stuff.persistence.api.Hql;

import java.util.Objects;

public class HqlFindByFieldValue implements Hql {

  private final Class aClass;
  private final String fieldName;
  private final String value;

  public HqlFindByFieldValue(Class aClass, String fieldName, String value) {
    this.aClass = aClass;
    this.fieldName = fieldName;
    this.value = value;
  }

  @Override
  public String getHql() {
    return String.format(
        "FROM %1$s AS E WHERE E.%2$s='%3$s'", aClass.getName(), fieldName, value);
  }

  @Override
  public boolean equals(Object o) {
    if (this == o) return true;
    if (o == null || getClass() != o.getClass()) return false;
    HqlFindByFieldValue that = (HqlFindByFieldValue) o;
    return Objects.equals(aClass, that.aClass)
        && Objects.equals(fieldName, that.fieldName)
        && Objects.equals(value, that.value);
  }

  @Override
  public int hashCode() {
    return Objects.hash(aClass, fieldName, value);
  }

  @Override
  public String toString() {
    return "HqlFindByColumnValue{"
        + "aClass="
        + aClass
        + ", fieldName='"
        + fieldName
        + '\''
        + ", value='"
        + value
        + '\''
        + '}';
  }
}
